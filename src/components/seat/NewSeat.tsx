            ...
            <div className='bg-white  drop-shadow-xs p-[24px] w-[40%] h-full rounded-[8px]'>
                ...
                <Form form={form} layout="vertical" onFinish={handleNew}>
                    <Form.Item label="Code" name="seatCode" rules={[{ required: true }]}>
                        <Input disabled={isPending} placeholder="Enter seat code" />
                    </Form.Item>
                    <Form.Item label="Name" name="seatName" rules={[{ required: true }]}>
                        <Input disabled={isPending} placeholder="Enter seat name" />
                    </Form.Item>
                    <Form.Item label="Price" name="price" rules={[{ required: true }]}>
                        <InputNumber addonAfter="%" disabled={isPending} style={{ width: "100%" }} placeholder="please input price rate" />
                    </Form.Item>
                    <Form.Item label="Description" name="description">
                        <Input.TextArea disabled={isPending} rows={3} placeholder="Enter seat description" />
                    </Form.Item>
                    <Form.Item>
